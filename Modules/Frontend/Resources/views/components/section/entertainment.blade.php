...
<div class="streamit-block">
    <div class="my-2 d-flex align-items-center justify-content-between me-2">
        <h5 class="mb-0 main-title text-capitalize">{{ $title }}</h5>
        @if($type != 'movies-pay-per-view' && $type != 'tvshows-pay-per-view')
            @if(count($data) > 6)

                @if(!empty($is_watch_list))
                    <a href="{{ route('watchList') }}"
                        class="flex-none view-all-button text-decoration-none"><span>{{__('frontend.view_all')}}</span> <i
