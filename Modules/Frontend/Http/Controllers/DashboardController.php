...
class DashboardController extends Controller
{
    ...
    public function PopularMovies2()
    {
        $cacheKey = 'popular_movie_2';
        $popular_movie = Cache::get($cacheKey);
        $html = '';
        if (!$popular_movie) {
            $popular_movie = [];
            ...
            $popularMovieIds = $popularSetting ? $popularSetting->value : null;
            // $sectionName = $popularSetting ? $popularSetting->name : __('frontend.popular_movie');
            $sectionName = 'AAAA Popular Movies';
        ...
        if (!empty($popular_movie)) {
            $html = view('frontend::components.section.popular_movies_2', ['data' =>  $popular_movie, 'title' => $sectionName ?? __('frontend.popular_movie'), 'type' => 'movie', 'slug' => 'popular_movie'])->render();
        }
